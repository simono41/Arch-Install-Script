...
function IMAGE() {
    ...
    then
        ...
        if cat /proc/mounts | grep $(pwd)/${work_dir}/${arch}/airootfs/var/cache/pacman/pkg > /dev/null; then
            echo "gemountet"
            umount $(pwd)/${work_dir}/${arch}/airootfs/var/cache/pacman/pkg
        else
            echo "nicht gemountet"
        fi

        if cat /proc/mounts | grep $(pwd)/${work_dir}/${arch}/airootfs > /dev/null; then
            echo "gemountet"
            umount $(pwd)/${work_dir}/${arch}/airootfs
        else
            echo "nicht gemountet"
        fi
...
function makeiso() {
    ...
    then
        ...
        then
            ...
            qemu-system-${arch} -enable-kvm -cdrom out/${imagename} -hda arch.img -hdb arch1.img -boot d -m 2048M -smp 4
        fi
