    ...
    then
        ...
        if [ "${mkinitcpio}" == "y" ]; then
            ...
            if [ "${version}" == "libre" ] || [ "${version}" == "lite" ] || [ "${version}" == "manjaro" ]; then
                ...
                echo "FILES=\"/etc/modprobe.d/blacklist-floppy.conf /etc/modprobe.d/blacklist-dell-smbios.conf\"" >> ${work_dir}/${arch}/airootfs/etc/mkinitcpio.conf
                ...
                echo "HOOKS=\"base udev archiso block filesystems keyboard plymouth\"" >> ${work_dir}/${arch}/airootfs/etc/mkinitcpio.conf
                echo "COMPRESSION=\"lz4\"" >> ${work_dir}/${arch}/airootfs/etc/mkinitcpio.conf
                echo "FILES=\"/etc/modprobe.d/blacklist-floppy.conf /etc/modprobe.d/blacklist_nouveau.conf /etc/modprobe.d/blacklist-dell-smbios.conf\"" >> ${work_dir}/${arch}/airootfs/etc/mkinitcpio.conf
